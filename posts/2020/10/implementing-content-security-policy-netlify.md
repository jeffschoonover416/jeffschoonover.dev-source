...
At this point everything is working except the inline CSS.  Having inline CSS is great for speed, but bad for security.  I would have thought just javascript would be a security threat, but inline CSS attacks can also be bad even though not as common.  Angular puts inline CSS, and even though they sanitize any input having lots of inline CSS makes a strict CSP difficult.  So my goal is to put as much of my CSS as possible into a separate file, and load only the inline CSS I need to for speed and protect it with hashes.  Fortunately, the Scully team just released a new plugin that does just that for Angular projects.  I'll add that to the project and finalize the CSP style-src directive in one of the next couple posts.  There are currently two inline scripts from Scully (one checks to see if Scully has been generated, and the other is the TransferState) whose hashes needed to be added to the CSP.  Since the state includes all the routes for the site, the transfer state script will change every time I write a new post.  The thought of updating the CSP every time I add a new post is giving new urgency to disabling Angular for this site and really minimizing Javascript!

So here's where we're going.  First I'm going to finish implementing the rest of the suggestions from the Mozilla security auditor, then install the Optimize CSS Scully plugin and see if I can't minimize the amount of inline CSS enough to use hashes for what I need and take the 'unsafe-inline' flag out.  In the future I'm going to figure out how to use CSS for the menu so I can disable Angular altogether, which will greatly simplify the CSP.
